...
void Adafruit_BMP085::initiateRawTemperature(void) {
  write8(BMP085_CONTROL, BMP085_READTEMPCMD);

}

uint16_t Adafruit_BMP085::readRawTemperatureFast(void) {
  // after delay of 5 ms from initiation 

  rawTemp = read16(BMP085_TEMPDATA);
  return rawTemp;
}
...
void Adafruit_BMP085::initiateRawPressure(void) {

  write8(BMP085_CONTROL, BMP085_READPRESSURECMD + (oversampling << 6));

}

uint32_t Adafruit_BMP085::readRawPressureFast(void) {
  // after delay of 26 ms from initiation for ultra high resolution 
...
int32_t Adafruit_BMP085::readPressureFast(void) {
  int32_t UT, UP, B3, B5, B6, X1, X2, X3, p;
  uint32_t B4, B7;
...
float Adafruit_BMP085::readAltitudeFast(float sealevelPressure) {
  float altitude;

  float pressure = readPressureFast();
